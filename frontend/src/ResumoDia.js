...
function ResumoDia() {
  ...
  return (
    <div style={containerStyle}>
      <h2 style={titleStyle}>Resumo do Dia ({hoje.split('-').reverse().join('/')})</h2>
              ...
              {agendamentos.slice(0, 3).map(a => (
                <li key={a.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                    <div>
                      <b>{a.hora}</b> - {a.nome} ({a.telefone})
                    </div>
                    <div>R$ {a.valor ? Number(a.valor).toFixed(2) : '-'}</div>
                  </div>
                  <div style={{ color: '#ccc', marginTop: 6 }}>{a.descricao}</div>
                </li>
              ))}
            </ul>
          )}
